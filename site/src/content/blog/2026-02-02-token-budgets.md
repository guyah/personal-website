...
<Tldr
	items={[
		...
		"Design for p95/p99, not the mean—tails create user-visible failures.",
		"A ‘~4 chars/token’ heuristic is useful only after you validate it on your own traffic.",
	]}
/>

A 32k context window sounds enormous… until you build a system that uses it.
...
…and suddenly you’re truncating the one part you cared about.
...
You can’t reason about budgets in characters. Tokenizers aren’t fixed-width.

But for rough planning, an English-ish heuristic is:

> **~4 characters per token** (including spaces/punctuation, averaged)

On real prompts (measured via API usage), here’s what the ratio looks like:
...
What matters operationally isn’t “do we have 32k tokens?”
...
<Insights
	title="What I actually do in production"
	items={[
		...
		"Compress history intentionally (summaries), don’t silently drop the latest user message.",
...
For a 32k window, a sane starting allocation:

- **System/tooling:** 1k–2k
- **Conversation history:** 6k–12k (summarize beyond that)
- **Retrieval:** 8k–14k (hard cap)
- **Response headroom:** 2k–6k (or you’ll clip the answer)

If you don’t set these caps, your system will set them for you—by truncating something important.
