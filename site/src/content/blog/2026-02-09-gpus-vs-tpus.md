...
description: "Architecture basics (NVIDIA-ish GPUs), memory hierarchy, tensor cores, batching, interconnects — and what they imply for training vs serving."
pubDate: 2026-02-09
...
This post is the mental model I wish I had before I started buying GPUs “because they’re fast”.
...
Everything else—tensor cores, systolic arrays, HBM, NVLink—exists to answer two questions:
...
- good for “weird” models and custom ops
- good ecosystem tooling

GPU traps:

- you can be compute-rich and still slow because you’re **memory-bound**
- you can have lots of FLOPs and still be slow because you’re **launch / sync / Python-bound**
...
- excellent throughput on “happy path” linear algebra
...
### “Other accelerators” (NPU, inference ASICs)
...
If you know what your production workload is *and it doesn’t change much*, these can win.

If you’re still experimenting, you may be buying constraints.

## The thing that actually decides performance: the memory hierarchy

Compute is cheap. Data movement is expensive.

The reason “GPU programming” feels like wizardry is that you’re not programming FLOPs—you’re programming **where the bytes live**.
...
- If you spill to host RAM over PCIe, you’re doing *distributed systems* now.
...
- small batches → low latency, low utilization
- large batches → high utilization, but queueing/p99 latency can get ugly
...
The minute your model doesn’t fit, or your throughput needs exceed one device, you care about:
...
- **Data parallelism** is training-friendly; for serving it’s usually just replication.

If your interconnect is weak, you’ll often do better with:
...
- Add a section on KV cache: why long context is “memory bandwidth + capacity” more than FLOPs.
